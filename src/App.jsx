...
export default function App() {
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-[#1a1a1a]">
        <form onSubmit={handleLogin} className="bg-white p-10 border-t-8 border-[#ff6600] w-96 shadow-2xl">
          <h1 className="text-2xl font-black italic mb-6 text-center tracking-tighter uppercase text-black">KARAMABUS KHENIFRA</h1>
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black animate-pulse">CHARGEMENT KARAMABUS...</div>;
    ...
    <div className="flex h-screen bg-[#f4f4f4] font-sans text-slate-900 print:block">
      <aside className="w-72 bg-[#1a1a1a] text-white flex flex-col border-r-4 border-[#ff6600] print:hidden">
        ...
        <nav className="flex-1 px-4 py-6 space-y-3 font-black text-[11px] uppercase">
          ...
          <button onClick={handleLogout} className="w-full text-left p-4 text-red-500 mt-10 hover:bg-red-500 hover:text-white rounded transition-all">🚪 Déconnexion</button>
        </nav>
      </aside>
      ...
      <main className="flex-1 p-10 overflow-auto print:p-0">
        ...
        {view === 'dashboard' && (
          <div className="space-y-8">
            <h2 className="text-3xl font-black border-b-8 border-[#ff6600] inline-block uppercase italic">Tableau de Bord</h2>
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 border-4 border-[#ff6600] text-center"><p className="text-[10px] font-black text-[#ff6600] uppercase">Articles Réserve</p><p className="text-5xl font-black text-blue-600">{inventory.filter(i => !i.bus_id && !i.en_panne).length}</p></div>
        ...
        {/* --- INVENTAIRE BUS (LECTURE POUR TOUS) --- */}
        {view === 'inventory' && (
          <div className="space-y-8">
            <div className="flex justify-between items-center border-b-8 border-[#ff6600] pb-2">
              ...
              <button onClick={() => window.print()} className="bg-black text-[#ff6600] px-4 py-2 font-black text-xs uppercase print:hidden">🖨️ Imprimer</button>
            </div>
            {busList.map(bus => (
              <div key={bus.id} className="bg-white border-4 border-black mb-10 break-inside-avoid shadow-xl">
                <div className="bg-black text-[#ff6600] p-4 font-black flex justify-between uppercase italic">
        ...
        {view === 'bus' && userRole === 'admin' && (
          <div className="space-y-10">
            ...
            <form onSubmit={handleAjoutCompletBus} className="space-y-6">
              ...
              <button type="submit" className="w-full bg-black text-[#ff6600] p-5 font-black uppercase text-xl border-4 border-black shadow-xl hover:bg-[#ff6600] hover:text-black transition-all">Installer Nouveau Bus</button>
            </form>
          </div>
        )}
        ...
        {view === 'maintenance' && userRole === 'admin' && (
          <div className="space-y-8">
            <h2 className="text-3xl font-black uppercase italic text-[#ff6600] border-b-4 border-black inline-block">Interventions</h2>
            <form onSubmit={validerEchange} className="bg-white p-8 border-4 border-black grid grid-cols-2 gap-4 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
              ...
              <textarea placeholder="Motif de la panne..." value={raison} onChange={e => setRaison(e.target.value)} className="col-span-2 border-2 p-3 h-20 font-bold uppercase" required />
            ...
            <table className="w-full bg-white border-4 border-black text-[10px]">
              <thead className="bg-black text-white uppercase italic">
                <tr><th className="p-3 text-left">Date</th><th className="p-3 text-left">Bus</th><th className="p-3 text-left text-red-500">Sortant</th><th className="p-3 text-left text-green-500">Entrant</th><th className="p-3">Raison</th></tr>
              </thead>
              <tbody>
                {logs.map(log => {
        ...
        {view === 'stock' && userRole === 'admin' && (
          <div className="space-y-10">
            <h2 className="text-3xl font-black uppercase border-b-8 border-black inline-block italic">Stock & Sorties</h2>
            <div className="bg-white p-6 border-4 border-black shadow-md">
              <h3 className="font-black uppercase mb-4 text-[#ff6600] text-xs">📥 Réception Matériel Neuf</h3>
            ...
            <div className="grid grid-cols-2 gap-8 items-start h-[500px]">
              <div className="bg-white border-2 border-black overflow-hidden shadow-lg h-full">
                <div className="bg-black text-[#ff6600] p-3 font-black uppercase text-xs italic">📦 Réserve Khenifra (Quantités)</div>
                <table className="w-full text-xs table-fixed">
                  <thead className="bg-slate-50 border-b-2 border-black font-black uppercase"><tr><th className="p-2 text-left">Désignation</th><th className="p-2 text-center w-24">Qté</th></tr></thead>
                  <tbody>
              ...
              <div className="bg-white border-2 border-red-600 overflow-hidden shadow-lg h-full">
                <div className="bg-red-600 text-white p-3 font-black uppercase text-xs flex justify-between items-center"><span>⚠️ Envoi Siège</span><button onClick={validerExpeditionSiege} className="bg-black text-white px-3 py-1 text-[8px] border font-black uppercase">Valider</button></div>
                <table className="w-full text-[10px] table-fixed">
                  <tbody>
                    {inventory.filter(i => i.en_panne).map(i => (
                      <tr key={i.id} className="border-b border-red-100"><td className="p-2 w-10 text-center"><input type="checkbox" className="accent-red-600" onChange={() => setSelectionEnvoi(prev => prev.includes(i.id) ? prev.filter(x => x !== i.id) : [...prev, i.id])} /></td><td className="p-2 font-black uppercase truncate">{i.nom}</td><td className="p-2 text-right font-mono truncate">{i.numero_serie}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
